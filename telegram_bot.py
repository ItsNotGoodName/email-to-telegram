...
class TelegramBot():
    def __init__(self, token, seconds_between_messages, chat_id):
    ...
    def send_message(self, message, disable_notification=False):
        ...
        try:
            self.updater.bot.sendMessage(chat_id=self.chat_id, text=message, disable_notification=disable_notification)
        except Exception:
    ...
    def send_photos(self, caption, paths):
        ...
        try:
            num_paths = len(paths)
            if(num_paths == 1):
                self.updater.bot.send_photo(chat_id=self.chat_id, photo=open(paths[0], 'rb'), caption=caption)
                ...
                self.updater.bot.sendMediaGroup(chat_id=self.chat_id, media=media)
            else:
